		...
		for (int linenum = 0; linenum < m_visible_lines; linenum++)
		{
			...
			else if (pitem.type() == menu_item_type::SEPARATOR)
			{
				...
				container().add_rect(
						...
						line_y0 + 0.5F * line_height() + machine().render().ui_target().texel_height() / 2.0F,
			...
			else if (pitem.subtext().empty())
			{
				...
				if (pitem.flags() & FLAG_UI_HEADING)
				{
					...
					container().add_rect(
							x1 + machine().render().ui_target().texel_width() * 3.0F,
							line_y0 + 0.5F * line_height() + machine().render().ui_target().texel_height() / 2.0F,
							visible_left + ((visible_width - heading_width) / 2) - lr_border(),
							line_y0 + 0.5F * line_height() - machine().render().ui_target().texel_height() / 2.0F,
							color,
							PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));

					container().add_rect(
							visible_left + visible_width - ((visible_width - heading_width) / 2) + lr_border(),
							line_y0 + 0.5F * line_height() + machine().render().ui_target().texel_height() / 2.0F,
							x2 - machine().render().ui_target().texel_width() * 3.0F,
							line_y0 + 0.5F * line_height() - machine().render().ui_target().texel_height() / 2.0F,
							color,
...
bool menu::do_handle()
{
	...
	if (render.ui_target().is_ui_rotated())
