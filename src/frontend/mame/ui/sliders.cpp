...
void menu_sliders::custom_render(void *selectedref, float top, float bottom, float x1, float y1, float x2, float y2)
{
	...
	if (curslider != nullptr)
	{
		...
		container().add_rect(
				...
				bar_top,
				bar_left + bar_width,
				bar_top + machine().render().ui_target().texel_height(),
		...
		container().add_rect(
				...
				bar_bottom,
				bar_left + bar_width,
				bar_bottom - machine().render().ui_target().texel_height(),
				ui().colors().border_color(),
				PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
		...
		container().add_rect(
				default_x,
				bar_area_top,
				default_x - machine().render().ui_target().texel_width(),
				bar_top,
				ui().colors().border_color(),
				PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
